...
# forked workers inheriting the ViT-H GPU memory layout.
try:
...
def load_config(path: str = "config_v1.yaml") -> dict:
    with open(path) as f:
...
CKPT_DIR = "models/weights/checkpoints_v1"
...
def save_checkpoints(encoder, online_agg, target_agg, optimizer, epoch: int, global_step: int = -1):
    """Save LoRA adapters + aggregator weights."""
    os.makedirs(CKPT_DIR, exist_ok=True)
    # Save LoRA adapters into separate directories to preserve the EMA gap
    encoder.backbone.save_pretrained(_ckpt("lora_online_latest"), selected_adapters=["online"])
    encoder.backbone.save_pretrained(_ckpt("lora_target_latest"), selected_adapters=["target"])
    ...
    shutil.copytree(_ckpt("lora_online_latest"), os.path.join(archive_dir, "lora_online_latest"))
    shutil.copytree(_ckpt("lora_target_latest"), os.path.join(archive_dir, "lora_target_latest"))
    shutil.copy2(_ckpt("agg_online_latest.pt"), archive_dir)
    shutil.copy2(_ckpt("agg_target_latest.pt"), archive_dir)
...
def load_checkpoints(encoder, online_agg, target_agg, optimizer, device):
    ...
    try:
        ...
        if os.path.exists(step_file):
            ...
            start_epoch += 1
            start_global_step = start_epoch * 2000
            print(f"Resuming from start of epoch {start_epoch+1} ...")
        
        # Load distinct EMA weights back into the respective adapters
        online_lora_ckpt = os.path.abspath(_ckpt("lora_online_latest"))
        target_lora_ckpt = os.path.abspath(_ckpt("lora_target_latest"))
        encoder.backbone.load_adapter(online_lora_ckpt, adapter_name="online")
        encoder.backbone.load_adapter(target_lora_ckpt, adapter_name="target")
...
def main():
    ...
    # BYOL IDENTITY: Target network MUST be a weight-clone of the Online network at Step 0.
    # Note: online_agg has an extra predictor head, so we use strict=False.
    target_agg.load_state_dict(online_agg.state_dict(), strict=False)

    # Sync LoRA adapters specifically
    online_params = {n: p for n, p in encoder.backbone.named_parameters() if ".online." in n}
    target_params = {n: p for n, p in encoder.backbone.named_parameters() if ".target." in n}
    for name_online, p_online in online_params.items():
        name_target = name_online.replace(".online.", ".target.")
        if name_target in target_params:
            target_params[name_target].data.copy_(p_online.data)
    ...
    # ---------------------------------------------------------------- training
    lora_params = [p for p in encoder.backbone.parameters() if p.requires_grad]
    agg_params  = list(online_agg.parameters())
    ...
    start_epoch, global_step = load_checkpoints(encoder, online_agg, target_agg, optimizer, device)
    ...
    for epoch in range(start_epoch, total_epochs):
        ...
        for step in loop:
            ...
            for accum_i in range(accum_steps):
                ...
                with torch.amp.autocast("cuda", dtype=torch.bfloat16):
                    ...
                    if not math.isfinite(loss.item()):
                        print(f"\n[WARNING] Corrupt batch detected (NaN/Inf loss). Skipping.", flush=True)
            ...
            if global_step > 0 and global_step % 500 == 0:
                print(f"\n[Checkpoint] Saving intermediate state at step {global_step}...")
                save_checkpoints(encoder, online_agg, target_agg, optimizer, epoch, global_step)
            # ------------------------------
